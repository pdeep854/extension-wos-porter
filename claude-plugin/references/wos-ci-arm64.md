...
- For Python / Node projects, prefer the platform-native toolchain (`cibuildwheel` for wheels, `npm rebuild` with `npm_config_target_arch=arm64`). The extension's [wos-build-recipes-python](wos-build-recipes-python.md) and [wos-build-recipes-nodegyp](wos-build-recipes-nodegyp.md) recipes cover the cross-install commands used inside the CI script.
