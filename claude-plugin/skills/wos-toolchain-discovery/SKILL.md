...
# ARM64 host fallback â€” allow x64-emulated toolset once, with a warning
...
- `$vsPath` â€” real directory under `Program Files\Microsoft Visual Studio\<year>\<edition>` â€” else BLOCKING.
- `$cl` â€” path ends in `$hostDir\cl.exe` â€” else BLOCKING (or the emulation-fallback warning above).
- `$msbuild` â€” file exists â€” else BLOCKING.
- `$dumpbin` â€” file exists â€” else BLOCKING (Phase 5/6 validation can't run without it).
