...
| **unsupported** | Explicitly x64-only per upstream, no ARM64 plan | Blocker — needs replacement, emulation, or ARM64EC |
...
| **Blocking** | Kernel driver, HAL-adjacent, or platform code that cannot be emulated | Hard stop — either replace, drop the feature on ARM64, or escalate to [Microsoft App Assure](https://learn.microsoft.com/en-us/microsoft-365/business/app-assure) |
...
|---|---|---|---|---|
| OpenSSL 3.x | vcpkg (`openssl:arm64-windows`) | native | ARM64 native | 1.1.x is EOL — force upgrade |
...
| CUDA / cuDNN | vendor | **unsupported** | Blocking | NVIDIA has no ARM64 Windows build — drop or gate |
...
Kernel-mode ARM64 porting is out of scope for `wos-porter` — flag every `*.sys` / `*.inf` targeting `NTx86` or `NTamd64` as `Blocking + escalate to App Assure`.
...
2. If not found, probe manifest metadata:
   ...
   - npm: `npm view <pkg> os cpu` — a `cpu` field missing `arm64` means no prebuilt.
   - PyPI: `pip download --platform win_arm64 --only-binary=:all: <pkg>` — no download means no wheel.
3. Classify runtime state:
   - `AA64` binary shipped → ARM64 native
   - Buildable from source with MSVC ARM64 → ARM64 native (mark as build-from-source)
   - Only x64 binary + no source → Emulated x64 (acceptable only for non-hot-path)
   - Kernel / HAL / hypervisor / Intel-only ISA → Blocking
...
| my_kmdf_driver.sys | — | in-tree | — | **Blocking** (kernel) | escalate: App Assure |
```
...
- **AppReady stages reached**: Assess ✓ | Build ✓ | Optimize ✓
- **WoA Ecosystem Dashboard status**: 12 native, 1 building (sharp 0.32.6 → rebuild), 0 unsupported, 0 unknown
